...
**Rev 5.48 · agosto 2026 · uso strettamente personale**
...
| 5.49 | 30 ago 2026 | **L'ultima modifica prima di chiudere la finestra si perdeva, e la diagnosi della rev 5.48 era FALSA.** Avevo attribuito la perdita al debounce di 500 ms del renderer contro il `dorme(400)` della prova. **Smentito misurando**: tre chiusure a confronto — con l'attesa, con `app.close()` di Playwright, con `BrowserWindow.close()` vera — hanno recapitato il marcatore tutt'e tre; il flush di `pagehide` funziona. Il difetto sta un piano sotto: `LayoutStore` frena a `MIN_INTERVALLO_S = 0.25` e **fonde invece di scartare**, tenendo in `_in_attesa` cio' che non scrive — e l'unico chiamante di `chiudi()` era lo spegnimento del CORE, che e' un servizio e resta acceso. La scrivania e' una finestra che si apre e si chiude sopra di lui: chi la chiudeva entro un quarto di secondo dall'ultima modifica la perdeva, perche' alla riapertura `messaggio_iniziale()` rilegge il DISCO. Due meta' scritte, provate (`TestIlFreno` chiama `chiudi()` a mano) e **mai congiunte**. La giunzione esisteva gia' cablata — `su_scrivania` — ed e' la stessa con cui il microfono segue la scrivania: `if quante == 0 and self._layout.chiudi()`. **A zero, non a ogni distacco**: con una finestra ancora aperta cio' che e' in attesa parte col messaggio successivo. Bocciature: il test era rosso prima che la riga esistesse, e rilassare la guardia a `quante >= 0` fa cadere il secondo. ⚠️ **Il sintomo da cui si era partiti — `agenti` che torna dopo essere stata tolta — NON e' chiuso**: col core corretto la prova va 6 verdi su 6, ma nel log del core si contano **18 chiusure di scrivania e 0 volte in cui il freno avesse trattenuto qualcosa**, quindi non e' questa la cura. Restano eliminate due ipotesi (il flush del renderer, il layout trattenuto) e nessuna trovata. Resta NON MISURABILE anche quanto spesso una persona chiuda la finestra entro 250 ms da un trascinamento | **§26.10**, §26.5 |
