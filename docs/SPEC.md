...
|---|---|---|---|
| 5.64 | 3 set 2026 | **Un rc 0 senza file, e l'invariante 32 dal vivo.** La prima bozza FreeCAD di opus — staffa a L, `if __name__ == "__main__"` in fondo come ogni script ben scritto — e' uscita con 0 e non ha scritto niente: FreeCADCmd da' a un file `__name__` uguale al nome del file, misurato nella sandbox. Il verificatore ha risposto `FALLITO, ASSENTI`; senza, sarebbe passata per un successo. Adesso FreeCAD gira via `runpy.run_path(..., run_name='__main__')`, che e' cio' che fa `python genera.py`. E lo storico della conferma ricorda anche quanti file ha prodotto e con quale comando: «uscita senza errori ma NON ha prodotto niente», e «script identico ma con un COMANDO diverso», perche' rc 0 da solo mentiva. 2302 test | **§17**, invariante 32 |\n| 5.63 | 3 set 2026 | **FreeCAD headless nella sandbox — decisione 4 di ADR-015, come binario nel profilo.** Su questa macchina FreeCAD e' uno snap, e `snap run` dentro bubblewrap muore su DBus (misurato). La via: il binario del snap con il **base snap come radice** in sola lettura, il snap e i content snap montati dove `meta/snap.yaml` li vuole, l'ambiente letto dal manifesto — bastano quattro variabili, misurato — e le case in una tmpfs. Stesse garanzie: niente rete, un solo `--bind`. `core/platform/linux_snap.py` (PyYAML, dichiarata). Il manifesto della bozza dichiara `interprete: python|freecad` e puo' produrre `.step`, verificato per intestazione ISO 10303-21 — debole e dichiarato. Tre errori sulla strada, tutti misurati: `/snap` e `/home` da aprire con una tmpfs, il symlink `current` da ricreare, `__pycache__` scritto nella bozza. `MeshPart` e `Draft` non partono nel pacchetto stesso. 2285 -> 2298 test | **§17**, **§20**, invariante 29 |\n| 5.62 | 3 set 2026 | **La conferma col diff.** Rieseguire uno script identico da' byte identici (misurato): l'unico motivo per rieseguire e' un cambiamento, ed e' quello che la conferma di §6.2 mette sotto gli occhi. Il piano di `esegui_bozza` dice prima esecuzione, identico (e «il risultato sara' identico», o «fallira' allo stesso modo» se l'ultima era fallita), o CAMBIATO con «+2/-2 righe» e il diff unificato fino a 60 righe. Lo storico — copia dello script e `esito.json` — sta fra i dati di JARVIS, non nella bozza. ⚠️ **Trovato scrivendo la fetta**: la finestra di conferma mostra il `dettaglio` SOLO delle operazioni senza percorsi, quindi interprete e sandbox della fetta 1 non erano mai comparsi sulla scrivania. Il piano ha ora quattro operazioni, `sandbox` e `diff` senza percorsi, e la finestra le disegna (pre-wrap per il diff). §11.7 su uno stato nuovo della galleria, `confirm-bozza`, col piano vero. ⚠️ Un accento grave in un commento CSS ha spento la galleria intera: la guardia c'era e non l'avevo eseguita prima dello scatto. 2280 -> 2285 test | **§6.2**, **§11.8**, **§17** |\n| 5.61 | 3 set 2026 | **Le righe del cervello sulla scrivania.** Mentre il modello scrive una bozza — tre minuti di silenzio, poi due frasi — il pannello del diario mostra che cosa dice e che cosa fa, riga per riga: il testo fra un passo e l'altro e i tool chiamati («scrive genera.py», «legge BOZZA.md»), dagli eventi `assistant` di stream-json e non da una barra inventata (invariante 23). `ClaudeT2.esegui(osserva=)` passa ogni evento mentre arriva; l'engine li scrive nel diario come DIALOGO con `chi="laboratorio"` e la traccia del turno, incorniciati da «opus scrive la bozza» e «bozza scritta in 181 s, 0,68 $». Mai detti a voce; il risveglio legge solo `azione` e non li racconta. Il pannello impara la terza voce — prima un `chi` sconosciuto finiva sotto «signore» — con `--txt-dim`, e il fixture porta righe registrate da un flusso vero (haiku, 47 s). §11.7 eseguito, audit pulito, densita' conforme. 2277 -> 2280 test | **§11.8**, **§17**, **§5.3** |\n| 5.60 | 3 set 2026 | **Gli occhi sulla cartella: l'osservatore delle bozze.** Dal «desktop Stark» che il proprietario ha portato entra l'idea buona — la viewport reagisce al disco — e non quella che non entra: si osserva il RISULTATO, mai si esegue il codice quando cambia, perche' un file in `bozze/` puo' averlo scritto T2 e l'innesco non distingue chi. `core/osservatore_bozze.py`: `bozze/*/*.stl` fermo per due giri e con byte diversi dall'ultima volta -> `model3d.preview`, con lo stesso `anteprima_di` di `esegui_bozza`; all'avvio niente. Un giro di `stat` in asyncio e non inotify, perche' i watch sono contati per utente e la scrivania ne consuma gia' abbastanza da far cadere nove test. Una riga di diario `anteprima_bozza` con traccia di sorveglianza, mai un guasto. Grado `osservatore_bozze`, `laboratorio.osserva_bozze`. 2265 -> 2277 test | **§17**, **§26**, **§20** |\n| 5.59 | 3 set 2026 | **Il laboratorio e' acceso nelle impostazioni del proprietario, e «esegui la bozza» e' una frase.** Con `laboratorio.enabled = true` e opus, la frase «costruisci nel laboratorio una staffa per un servo SG90» ha prodotto il primo pezzo vero in `~/JARVIS/laboratorio/bozze/`: 181 s, 0,68 $, 1.628 triangoli, 36x26x28 mm, `RIUSCITO` — con la conferma data dal proprietario col piano risolto davanti. ⚠️ Non dalla voce: dalla scrivania il testo non entra (invariante 31), quindi il diario del core non ha quella riga. L'altra meta' del laboratorio e' l'intento `riesegui_bozza`: «esegui la bozza», «riesegui la bozza della staffa», «rilancia l'ultima bozza» — la radice risolve il NOME (l'ultima toccata, o la piu' recente che contiene la coda) e passa da `esegui_bozza` con la stessa conferma; vale per una bozza scritta a mano. A voce JARVIS dice l'etichetta, non `2026-09-03-...`. 2224 -> 2265 test | **§7.6**, **§17**, **§20** |\n| 5.58 | 3 set 2026 | **`manifold3d` e `shapely` entrano, col si' del proprietario.** Il primo script del laboratorio dal vivo era caduto su `ModuleNotFoundError: manifold3d`; senza booleane un foro non coassiale si scrive a mano. Chieste nel resoconto, approvate, aggiunte con `uv add` (Apache-2.0 e BSD-3, non un secondo motore 3D) e **misurate dentro `Profilo.LABORATORIO`**: radice vuota, venv in sola lettura, wheel compilate — `difference` e `extrude_polygon` watertight, STL riletti con `struct`. ⚠️ Il prompt di chi scrive diceva «senza booleane si costruisce a mano» anche quando mancavano SOLO scipy e networkx: i consigli seguono le assenze una per una, e un test lo tiene. Dal vivo: la staffa a L con due fori, che senza booleane non era uscita in dieci minuti, e' uscita in 104 s con sonnet, `RIUSCITO`, 412 triangoli. 2221 -> 2224 test | **§17**, **§20** |\n| 5.57 | 3 set 2026 | **Il laboratorio — ADR-015 approvato e fatto, e «vietato» era la parola sbagliata.** Il proprietario ha corretto la premessa: eseguire codice generato dall'LLM non e' vietato, e' «chiedi prima» piu' sandbox (riga 104 di `CLAUDE.md`, ADR-006). Le quattro decisioni sono sue: `~/JARVIS/laboratorio/`, `llm.laboratorio_model = "opus"` separato da `t2_model` e **mai haiku** (lo schema lo rifiuta), due zone — T2 e la sandbox scrivono solo in `bozze/` — e `python` prima di FreeCAD e Blender. ⚠️ **Il primo criterio ha cambiato il disegno**: la regola della zona non e' «T2 senza `Bash`», perche' `core/llm/claude_t2.py` aveva misurato che `--allowedTools` non e' un confine; e' `Profilo.AGENTE`, bubblewrap con la sola bozza scrivibile, misurato con Claude Code vero (sonnet, 148 s, $0,28, niente toccato fuori). `Profilo.LABORATORIO` e' `CODICE` piu' un `--bind`, col venv di JARVIS in sola lettura. Il tool `esegui_bozza` prende un NOME, mai un percorso; il verificatore rilegge gli STL dichiarati in `bozza.json` con `struct`. ⚠️ **Il primo script dal vivo e' caduto** su `ModuleNotFoundError: manifold3d`: il mio prompt prometteva le booleane e il venv non le ha. Il prompt adesso SONDA le librerie con `find_spec`. L'invariante 34 e' delimitato da ADR-015 in §20. 2180 -> 2221 test | **§20**, **§17**, **§6.2** |\n| 5.56 | 3 set 2026 | **Il tubo e' stato TOLTO, e la domanda che mancava era «a che cosa serve».** Ho scritto §17.4 ② due volte — spline chiusa, poi tubo piegato — e la seconda era un pezzo vero, con misure di progetto e le sue quote. Restava senza un uso. Chiedendolo, la risposta e' che il proprietario fa **prop e meccanica da stampare**: un elmo, e le staffe per i servocomandi che ci vanno dentro. Un tubo non c'entra, e un generatore senza un uso e' il modo di morire n. 1 di `ANALISI-SENIOR` §7 — «componenti che non controllano nulla» — che ADR-014 cita nella propria sezione dei rischi e che io ho fatto lo stesso, due volte. ⚠️ **L'errore vero e' nella domanda che avevo posto**: «`model3d.py` e' nel progetto o esce dalla SPEC?», cioe' una questione di coerenza fra documenti. Per il resoconto del mattino avevo chiesto A CHE COSA SERVE OGNI GIORNO, e quella fetta e' reale. Qui no. ⚠️ E togliendolo, `scripts/orfani.py` ha trovato la conseguenza: `segmenti_per`, la meta' Python di `segmentsFor()`, restava «provata mai congiunta» — il tubo era il suo unico chiamante di produzione. Tolta col suo test gemello: mezzo ponte e' peggio di nessun ponte, ed e' la stessa disciplina che aveva fatto CONGIUNGERE `bbox_combacia` invece di dichiararla. La regola §11.10 regola 2 resta imposta in `ui/src/three/`, dove la usano anelli, quadranti, globo e nuvola di punti. Restano di quella fetta la `tolleranza_mm` con la ragione obbligatoria e le **quote scelte dal generatore**. 2207 -> 2180 test | **§17.4**, **§17** |\n| 5.55 | 3 set 2026 | **Il tubo di §17.4 ② e' stato RIFATTO: la spline chiusa faceva una cosa che non era un pezzo.** La prima stesura eseguiva §17.4 ② alla lettera e la matematica era giusta — punti di controllo a 1,5e-14 mm, telaio chiuso senza cucitura, toro — ma l'oggetto era un anello ondulato con misure RISULTANTI (214,9 x 202,0 x 67,6) e un'asimmetria che si leggeva come un errore: l'opposto di §11.10 regola 4. Respinto guardandolo, e §11.7 dice che una violazione si riscrive. Adesso e' un tubo piegato: corse dritte, pieghe a raggio costante, corsa-rotazione-angolo come su una piegatrice, misure tonde perche' di progetto. La macchina resta tutta — sezione spazzata, telaio a torsione minima, densita' dalla curvatura, gemello `segmenti_per`/`segmentsFor()` inchiodato da un test — e se ne va la CHIUSURA del telaio, che teneva una proprieta' che un percorso aperto non ha. ⚠️ **Il bbox non e' «la linea d'asse piu' il raggio»**, e il presidio di regola 7 l'ha preso subito con 7,8 mm su X: il disco della sezione sta nel piano perpendicolare alla tangente, e la forma esatta e' `r*sqrt(1 - t_k^2)` — dalla sola tangente, quindi una seconda affermazione indipendente. ⚠️ **Le quote le sceglie il GENERATORE, non il pannello.** Il pannello annotava i tre lati del bounding box: sulla piastra e' il pezzo, su un tubo piegato sono un risultato appeso ad angoli nel vuoto. Adesso la piastra dichiara i suoi tre lati e il foro, il tubo Ø12, R24 e lo sviluppo. ⚠️ **E il raggio di piega SEGUE il diametro**, trovato provando la frase vera: «un tubo da 20 millimetri» falliva perche' il diametro cambiava e la matrice no. Otto sabotaggi, e l'ottavo non ha trovato niente da rompere: `-k quote` deselezionava tutti e 63 i test, cioe' il meccanismo era costruito e non sorvegliato. 2174 -> 2205 test | **§17.4**, **§11.10**, **§7.6** |\n| 5.54 | 3 set 2026 | **§17.4 ② fatto: il tubo su spline, e la regola della densita' scritta DUE volte.** `segmenti_per` in Python e `segmentsFor()` in JavaScript sono la stessa formula in due linguaggi, e §17.2 obbliga ad averle entrambe — il generatore nel core, il componente che lo incassa nel renderer. Non se ne cancella una: si eseguono insieme su dodici ingressi scelti dove due implementazioni divergono, e un test le inchioda. La curva e' scritta per esteso (Barry-Goldman, centripeta) perche' §11.10 regola 5 vieta `THREE.CatmullRomCurve3`; passa per i punti di controllo a 1,5e-14 mm. Il telaio e' a torsione minima e **si chiude**: l'angolo residuo dopo un giro si distribuisce sugli anelli, e senza la cucitura vale 118,77°. `Modello` guadagna `tolleranza_mm` sul bbox **con la ragione obbligatoria** — la sezione e' un poligono inscritto, il dichiarato e' il cilindro circoscritto, e la differenza ha la forma chiusa `2r(1-cos(pi/lati))`, 0,272 mm su 215. Il confronto e' a senso unico: il dichiarato puo' stare SOPRA i vertici, mai sotto. ⚠️ **Una bocciatura su cinque e' uscita VERDE**: sostituendo i nodi centripeti con nodi uniformi, i test sui punti di controllo e sulla topologia restavano verdi. E misurando che cosa la centripeta comprasse, il numero ha smentito il commento che avevo scritto — 2,72 mm di scarto contro 2,92 dell'uniforme su questo guscio, e **peggio** su uno piu' duro (10,16 contro 9,45), perche' i punti sono equispaziati su una formula liscia e la patologia da cui protegge non si presenta. Il commento e' stato corretto e il presidio fissa la PARAMETRIZZAZIONE, non una proprieta' che qui non consegna. ⚠️ **E un difetto letto in uno scatto che non c'era**: la quota della profondita' sembrava cadere fuori dal riquadro, ho scritto il rientro, e misurando i rettangoli con e senza le quote fuori sono zero in entrambi i casi — tre pixel dal bordo sono dentro. Il rientro e' stato TOLTO. §11.7 regola 4 vale in tutt'e due i versi. Il presidio del quasi-comando passa dal fissare la percentuale al fissare i NOMI degli otto falsi positivi: il denominatore cresce col corpus e il numeratore no. 2174 -> 2203 test | **§17.4**, **§11.10**, **§7.6** |
...
| 5.48 | 31 ago 2026 | **Il nucleo si rifa' sul riferimento HUD misurato, e cinque regole sono derogate.** Decisione del proprietario: la replica del suo riferimento vince sulla specifica dove i due sono in contrasto — invariante 19 (glow e bloom, con esenzione **nominata** nell'audit per il solo nucleo), §25.11 (three.js per il globo L5), invariante 25 con §10.3 («fondo: immobile», la sola riga mai violata finora), §10.6 (classe 2 fuori da un pannello) e il tetto di §25.5. Cio' che NON e' derogato: invariante 18 — la palette misurata entra in §10.1 come **tre gradini**, non come letterali, e cinque degli otto livelli del riferimento erano gia' qui — invariante 23 (telemetria vera, nessun «APOGEE: 420.5 KM»), invariante 9 (anime.js, niente GSAP), invariante 22, invariante 1 (il PCM resta nel core, sul filo passano le bande). ⚠️ Il riferimento e' disegnato per 1024x1024 e il nucleo vive in Ø326: i **raggi** si conservano come rapporti, la **tipografia** si dimensiona in unita' di viewBox per cadere sui gradini `--t-*` veri. Deroghe, misure e costo del ritorno in `docs/acceptance/NUCLEO-AURORA.md` (il referto di allora, `NUCLEO-HUD.md`, e' stato cancellato il 2 set 2026: sta nella storia di git, commit `427e48c`) | **§10.1**, §10.3, §10.6, §25.5, §25.11 |
| 5.47 | 29 ago 2026 | **`implementazione_di_protocollo` scusava per NOME NUDO.** Un `Protocol` e' strutturale — chi lo implementa non lo eredita — e l'indice teneva i soli nomi dei suoi membri: qualunque classe con un metodo omonimo diventava benigna con la spiegazione «X e' dichiarato da un protocollo di `core/`», vera alla lettera e falsa nella sostanza. Adesso la categoria chiede la compatibilita' STRUTTURALE — tutti i membri pubblici del protocollo — che e' l'unica verifica possibile, e la ragione NOMINA il protocollo invece di lasciarlo cercare. ⚠️ **Effetto sull'elenco: ZERO**, e va detto invece che vantato: 531/174/5 prima e dopo, nessuna voce comparsa, sparita o riclassificata. Le quattro `implementazione_di_protocollo` di `core/` — i quattro `aclose` dei provider — implementano davvero il loro protocollo: la spiegazione era vera **per caso**, e adesso lo e' per costruzione. ⚠️ E la misura che aveva motivato il turno era in vitro: `Diario.leggi` non entra affatto nell'elenco, perche' `self._ocr.leggi(png)` (`core/vision/argus.py:102`) e' un `Attribute` che il conteggio per nome le accredita — la coprivano DUE limiti in fila, e questo era il secondo, dietro il primo | §5.29 |
| 5.46 | 29 ago 2026 | **La striscia LAYOUT diceva `ok` su un file che il core aveva appena buttato via.** La formula del dock e' `corrotto_in ? "corrotto" : esiste ? "ok" : "assente"`, e `corrotto_in` si valorizza dentro `carica()` — che aveva **un solo chiamante**, `messaggio_iniziale`, cioe' girava quando la scrivania si collega. Ma `state.snapshot` parte PRIMA (§6.3: «ogni client riceve lo stato completo prima di qualunque delta»), quindi nella sessione in cui il guasto accade nessuno poteva saperlo. **Misurato**: file corrotto, prima di `carica()` `esiste=True, corrotto_in=None` -> «ok»; dopo, `esiste=False, corrotto_in=<path>` -> «corrotto». ⚠️ **Le due cure che sembravano le uniche pagavano un prezzo**: leggere il file `.corrotto` da disco avrebbe cambiato il significato di `corrotto_in` da «e' successo adesso» a «e' successo, una volta»; spostare lo snapshot avrebbe rotto `jarvis doctor`, che prende il PRIMO frame per lo snapshot (`core/doctor.py:56`). La cura vera e' all'origine: **il core legge il proprio layout all'avvio**, non alla prima connessione. Non cambia il significato di niente, non tocca l'ordine dei messaggi, e distingue i tre casi che prima si confondevano in due — corrotto, assente, sano. ⚠️ Resta scoperta e dichiarata una finestra stretta: un file che si corrompe FRA l'avvio e la connessione; chiuderla vorrebbe dire rileggere il disco a 2,5 Hz | **§26.10**, §6.3 |
| 5.45 | 29 ago 2026 | **Due orfani nuovi, e in tutt'e due i casi la ragione che si voleva scrivere era FALSA.** `Diario.giorni` si firma — ma non per il motivo proposto: la ragione diceva «vale anche per `Diario.leggi`, coperta dall'omonimia con `ocr.leggi`», e **misurato con lo scanner vero, `Diario.leggi` senza alcun chiamante torna `implementazione_di_protocollo`: benigna, con una spiegazione falsa**, perche' `leggi` e' dichiarato da `Ocr` e lo scanner scusa per NOME NUDO ogni metodo omonimo di un membro di protocollo senza verificare che la classe lo implementi. E' un secondo difetto dello strumento, diverso dal conteggio per nome chiuso nella rev 5.44, ed e' **dichiarato e non chiuso**. ⚠️ **In `core/` nessuno legge il diario**: cinque `annota()` e nient'altro; il pannello e' una coda VIVA che riceve `agent.diario` mentre le righe si scrivono e non apre nessun file — riaprendo l'app, il diario di ieri non si vede. Due affermazioni scritte erano false e sono corrette. **`Layout.vuoto` si TOGLIE**: tre `len()` mascherati da predicato, sei chiamanti tutti nei test, e la risposta esiste gia' in `ui/src/app.js:300-303` con una forma diversa. ⚠️ Ma la cura proposta finiva con «scrivere in `test_10` che copre §26.5», ed e' falso: `test_11` esige `ripristino["messi"]`, scritto solo dopo `if (!roba) return`, e una cartella aperta E' un pannello — il caso `pannelli == 0` non e' MAI esercitato. La proprieta' **resta senza custode**, dichiarata in `docs/acceptance/IL-FONDO-SENZA-CUSTODE.md` con la bocciatura che lo renderebbe tale: una ricevuta falsa vale meno di un buco dichiarato | §3.2, §26.5, §5.29 |
| 5.44 | 29 ago 2026 | **Lo scanner degli orfani contava i richiami per NOME, e un nome comune ne nascondeva uno vero.** Due volte in un giorno: `Isteresi.gesto` era classificata benigna perche' `def alimenta(self, gesto)` usa cinque volte il suo PARAMETRO, e `tracker.percorso_modello` e' sparita dall'elenco nell'istante in cui e' stata scritta una `PhraseWake.percorso_modello` altrove (misurato: 170 -> 167 orfani con una definizione in piu'). ⚠️ **E il punto cieco non aveva UNA SOLA prova**: era una frase nell'intestazione del modulo, e chiunque poteva cancellarla senza che niente diventasse rosso. Adesso `riferimenti()` fa un'analisi **per scope**: un `ast.Name` che si risolve a un legame locale — parametro, variabile, bersaglio di `for`, di `with as`, di una comprensione — non e' un richiamo. **532 def, 168 -> 175 orfani, 4 -> 7 sospetti**, zero spariti; scansione 0,32 -> 0,53 s contro un tetto di 5. I tre sospetti nuovi sono orfani VERI, verificati a mano: `Diario.giorni` (un solo chiamante, in `scripts/`), `Layout.vuoto` (sei, tutti nei test), `Isteresi.gesto` (zero, ovunque). ⚠️ **Tre regole che senza le quali lo scanner INVENTA orfani**, e due le ha trovate la bocciatura prima di me: il corpo di una classe si esegue dall'alto in basso, lo scope di classe non e' visibile dai metodi annidati, e **un import NON lega** — legandolo, trentadue classi di produzione fra cui `ClaudeT1`, `VoicePipeline` e `PhraseWake` risultavano orfane. ⚠️ **Il limite che RESTA e' dichiarato e adesso ha un test**: gli `ast.Attribute` si contano ancora per nome, e chiuderlo vorrebbe dire inferenza di tipi. Misurate 52 collisioni — 147 definizioni su 532, in 45 moduli su 71 | **§5.29** |
| 5.43 | 29 ago 2026 | **Il consolidamento saltava una sessione e non la rivedeva mai piu', e non e' un'ipotesi: UNA GIORNATA DI MEMORIA E' GIA' PERSA.** `esegui()` partiva da `ultimo_run()` e filtrava i turni con `sessioni_dal(da)`, ma quel timbro e' un **orologio di parete scritto a fine ciclo**: ogni giro saliva sopra il `ts` delle sessioni che NON aveva consolidato — quella caduta sul ramo `not r.ok`, quella lasciata a meta' da un crash — e le rendeva invisibili per sempre. **Misurato sul disco vero**: `sessions/2026-08-27.jsonl` porta 7 turni con `ts` fino a 1787853324, il timbro dice 1787882411 (le 04:00 del 28), turni visibili al giro dopo **zero**, e in `topics/` c'era solo la sessione del 26. ⚠️ **La cura ovvia era peggiore del male**: spostare `_segna_run()` dentro il ciclo scrive l'ora di adesso, maggiore del `ts` di ogni sessione non ancora lavorata — un doppione rumoroso diventa una perdita silenziosa, e la suite resta verde. **Cura: la frontiera non e' piu' un numero ma un INSIEME DI NOMI** — le sessioni senza una riga in `initiatives/`, il registro che chi consolida scrive gia'. Con lei spariscono le tre domande che il confronto fra `ts` portava: `>=` o `>`, l'ordine delle sessioni, il turno scritto mentre il giro gira. `ts` non si confronta piu' con niente. **Misurato sulla copia del disco vero: il giro successivo consolida il 27 E il 28 — la giornata persa torna — e il secondo giro non fa niente.** ⚠️ Prezzo dichiarato: la sessione di OGGI si lascia stare finche' e' aperta | **§5.5** |
| 5.42 | 29 ago 2026 | **`jarvis doctor` diceva `ok` su un modello di richiamo che il riconoscitore non aveva mai aperto.** `set_frasi()` cambia la grammatica e NON ricarica il modello — 206 ms misurati, ed e' la scelta giusta — ma lo snapshot rispondeva con `str(s.voice.wake.model)`, cioe' l'IMPOSTAZIONE: cambiando `voice.wake.model` in `settings.toml` il campo diventava il percorso nuovo all'istante, §16.1b verificava che QUEL file esistesse e rispondeva `ok`, mentre il riconoscitore continuava col modello di prima **fino al riavvio**, e nessuno lo diceva. Adesso lo snapshot porta il modello VIVO (`PhraseWake.modello_caricato_da`) accanto a quello chiesto, la divergenza vale `fail` — stessa regola di `_check_unit` — e il ricaricamento delle impostazioni la ANNUNCIA su `agent.advisory`. ⚠️ L'annuncio sta PRIMA del cancello «le frasi sono le stesse»: chi cambia solo il modello non tocca le frasi, ed e' il caso da cui viene il difetto. ⚠️ E il nome della proprieta' e' `modello_caricato_da` e non `percorso_modello` perche' quello **nascondeva un orfano**: `core/gestures/tracker.py:97` ha una funzione con lo stesso nome, e lo scanner conta per NOME — misurato, `170 -> 167` orfani con una definizione in piu' | **§7.2**, **§16.1b** |
...
:root {
  ...
  /* ⚠️ TRE GRADINI NUOVI, e vengono da una MISURA sull'immagine di
     riferimento del nucleo HUD — non da un gusto.

     `docs/design-reference/` porta l'analisi forense di quell'immagine: profilo
     radiale e clustering dei colori su 1024x1024. La palette che ne esce ha
     OTTO livelli di ciano; la nostra ne aveva cinque, e quattro degli otto
     cadevano in buchi della rampa. Luminanza Rec. 709 su 0-255, la stessa
     formula di `scripts/densita.mjs`:

       misurato    L      dove cade da noi
       #111F28    28,7    == --bg-panel (30,7)     si riusa
       #205463    74,0    buco fra 900 e 700       -> --cy-800
       #2F6575    90,7    == --cy-700 (99,6)       si riusa
       #5A9AAB   141,6    buco fra 700 e 500       -> --cy-600
       #77C3D5   180,1    == --cy-500 (181,4)      si riusa
       #94E5F4   212,9    buco fra 300 e 200       -> --cy-200
       #FF2D2D    89,6    == --rust, stesso ruolo  si riusa

     Cinque su otto erano gia' qui: si aggiunge il minimo, non la palette
     intera. Il sesto livello del riferimento — #407D8F, L 113 — NON entra: sta
     fra --cy-800 e --cy-600 e si raggiunge con l'opacita', che e' il modo in
     cui il riferimento stesso dice di costruire la gerarchia («un solo hue,
     gerarchia da luminosita' E OPACITA'»). Se il confronto visivo mostrera'
     che serve davvero, si aggiunge allora, con la misura che lo chiede.

     ⚠️ Il confronto visivo E' stato fatto — il nucleo HUD, 1º settembre 2026 —
     e il gradino NON e' servito: la forbice di §25.13.5 si e' chiusa con
     --cy-600 in otto stati e --cy-500 nel nono, due gradini che c'erano gia'.
     La riserva resta in piedi per la prossima volta che qualcuno la rilegga.

     La numerazione segue quella che c'era: numero che scende, luminanza che
     sale. La rampa resta monotona — 48,5 · 74 · 99,6 · 141,6 · 181,4 · 200,4 ·
     212,9 · 231,3 — e nessun gradino esistente si sposta di un digit.

     ⚠️ `--cy-200` STA SOPRA IL TETTO DI §25.5, che capa il nucleo a --cy-500
     (L 181). E' una deroga dichiarata: il riferimento usa quel livello per il
     testo del logo e per i picchi della waveform, e senza non e' replicabile.
     Il vincolo che §25.5 difende — il nucleo non compete col dato — regge lo
     stesso: --cy-100 (L 231) resta vietato nel nucleo ed e' il livello del
     testo dei pannelli. Costo e ritorno in `docs/acceptance/NUCLEO-AURORA.md`. */
...
**Della fetta 2 restano due cose e non il suo generatore.** `Modello` ha una
`tolleranza_mm` sul bbox **con la ragione obbligatoria**, e le **quote**, che
le sceglie il generatore e non il pannello — annotare i tre lati del bounding
box funziona su una piastra e non su una forma il cui ingombro sia un
risultato. Il tubo che le aveva portate è stato **tolto** (§17.4 ②): non
c'entrava con ciò che il proprietario stampa. Con lui se n'è andata la metà
Python di `segmentsFor()`, che senza un generatore curvo nel core era «provata,
mai congiunta» — `scripts/orfani.py` l'ha detto, e mezzo ponte è peggio di
nessun ponte. La regola §11.10 regola 2 resta imposta dove si applica,
`ui/src/three/`.
...
> ⚠️ **NON IMPLEMENTATO, e la ragione non è il tempo — 3 settembre 2026.**
> Questo punto è stato scritto **due volte e tolto**, e il secondo giro ha
> trovato il motivo vero: nessuno sa a che cosa serva un tubo in questo
> progetto. Il proprietario fa **prop e meccanica da stampare** — un elmo, le
> staffe per i servocomandi che ci vanno dentro — e un anello o una linea
> idraulica non c'entrano. Un generatore senza un uso è la stessa cosa di un
> componente che non controlla nulla (`ANALISI-SENIOR` §7①). Ciò che segue
> resta come cronaca di che cosa era stato fatto, e sta ai commit `cd5dbbd` e
> `a6c79db`.
>
> ⚠️ **EMENDATO e poi RITIRATO: non era una spline chiusa, era un tubo piegato.** `core/model3d/tubo.py` ha prima fatto ciò che questa riga dice alla lettera — spline Catmull-Rom chiusa su due armoniche, spazzata in tubo — e la matematica era giusta: passava per i punti di controllo a 1,5·10⁻¹⁴ mm, il telaio si chiudeva senza cucitura, la topologia era un toro. **L'oggetto non era un pezzo**: misure risultanti invece che di progetto (214,9 × 202,0 × 67,6 mm) e un'asimmetria che si leggeva come un errore, cioè l'opposto di §11.10 regola 4. Respinto guardandolo, e §11.7 dice che una violazione si riscrive: sta al commit `cd5dbbd`. Adesso è **corse dritte raccordate da pieghe a raggio costante** — corsa, rotazione, angolo, come si programma su una piegatrice — con misure tonde perché di progetto. La matematica dei segmenti ci sta meglio: `segmentsFor(raggio, arco)` è nata per un arco di cerchio, e qui gli archi sono cerchi veri. L'esito era in `docs/acceptance/MODELLO-3D-TUBO.md`, cancellato col generatore.
