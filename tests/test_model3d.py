...
class TestIlToolChiedePrimaDiScrivere:
    def test_non_esiste_un_argomento_PATH(self) -> None:
        ...
        # I parametri delle forme dell'allowlist, e nient'altro: uno schema che
        ...
        attesi = {"forma"}
        for genera in GENERATORI.values():
            attesi |= set(genera().params)
        assert campi == attesi, campi
...
class LaRegolaDellaDensitaVIVEnelRENDERER:
    """⚠️ Qui c'era `TestIlGemello`, e con lui `segmenti_per` in Python.

    Erano la stessa formula di `ParametricComponent.segmentsFor()` in due
    linguaggi, tenute uguali da un test che le eseguiva entrambe: §17.2 mette
    il generatore nel core e il componente che lo incassa nel renderer, e la
    regola della densità è dell'uno e dell'altro.

    Poi il tubo — l'unico generatore del core con una superficie curva — è
    stato tolto, e `scripts/orfani.py` ha detto la cosa giusta: la metà Python
    era «provata, mai congiunta». Tenerla sarebbe stato **mezzo ponte**: una
    formula che nessuno chiama, più un test che la lega a una formula che
    invece gira. È la stessa disciplina che ha fatto congiungere
    `Modello.bbox_combacia` alla costruzione invece che dichiararla.

    La regola non è sparita: vive dove si applica, cioè in
    `ui/src/three/`, e la usano anelli, quadranti, globo e nuvola di punti. Il
    giorno in cui un generatore del core farà una superficie curva, la metà
    Python torna **nella stessa fetta che la chiama**, col suo test gemello.
    Sta al commit `a6c79db`.
    """

    def test_la_regola_e_ANCORA_imposta_dove_si_applica(self) -> None:
        import re as _re

        usi = 0
        for f in (RADICE / "ui/src/three").rglob("*.js"):
            usi += len(_re.findall(r"segmentsFor\(", f.read_text(encoding="utf-8")))
        assert usi >= 3, (
            f"segmentsFor() usata {usi} volte: §11.10 regola 2 non è più imposta "
            "da nessuna parte")

    def test_e_nessuno_l_ha_riscritta_nel_CORE_senza_chiamarla(self) -> None:
        """Il verso opposto: se qualcuno la riporta in Python, deve avere un
        chiamante di produzione e il test gemello — o `scripts/orfani.py` la
        ritroverà come l'ha trovata questa volta."""
        py = "\n".join(f.read_text(encoding="utf-8")
                       for f in (RADICE / "core").rglob("*.py"))
        assert "def segmenti_per(" not in py, (
            "la metà Python è tornata: serve un chiamante di produzione e il "
            "test che la esegue insieme a `segmentsFor()` (commit a6c79db)")


class TestIlCatalogoEUNAALLOWLIST:
    """⚠️ Qui c'era `TestLeDueFormeConvivono`, e le forme erano due. Il tubo è
    stato TOLTO il 3 settembre 2026: non c'entrava con ciò che il proprietario
    stampa davvero — prop e meccanica — e un generatore senza un uso è la
    stessa cosa di un componente che non controlla nulla. Sta in git.

    Ciò che resta provato è la proprietà, non il numero: il catalogo è chiuso,
    ogni nome ha un generatore e dei conteggi, e un nome inventato non entra.
    """

    async def test_ogni_forma_arriva_al_DISCO_e_al_verdetto(self, mondo) -> None:
        for forma in GENERATORI:
    ...
    async def test_un_parametro_che_la_forma_non_conosce_si_rifiuta(self, mondo) -> None:
    ...
    async def test_una_piastra_non_ha_niente_da_DEROGARE(self, mondo) -> None:
        """Il suo bbox è esatto: gli smussi tagliano verso l'interno e non
        spostano gli estremi. La tolleranza dichiarata serve a una forma che
        discretizza una superficie curva, e oggi non ce n'è."""
